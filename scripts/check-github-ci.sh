...
#   --skip-workflows       Poll only --jobs (no Android CI/Security Scan/CodeQL rollup); requires --jobs
...
resolve_required_workflows() {
  REQUIRED=()
  if [ -n "${GITHUB_REQUIRED_WORKFLOWS:-}" ]; then
    IFS=',' read -ra REQUIRED <<< "${GITHUB_REQUIRED_WORKFLOWS}"
    return
  fi
...
workflow_file_for() {
  case "$1" in
    "Android CI"|CI) echo "android.yml" ;;
...
while true; do
  ...
  if [ -n "$JOBS_CSV" ]; then
    WF_FOR_JOBS="${REQUIRED[0]:-CI}"
    RUN_ID="$(gh run list --repo "$REPO" --commit "$REF" --workflow "${WF_FOR_JOBS}" --json databaseId -q '.[0].databaseId' 2>/dev/null || true)"
    if [ -z "$RUN_ID" ]; then
      echo "WAIT ${WF_FOR_JOBS} jobs: no run yet"
      PENDING=$((PENDING + 1))
    else
