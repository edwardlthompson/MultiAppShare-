# Completed tasks (Milestones A–AE)
...
## Milestone AE — Product backlog (allideas 2026-08-30) (2026-08-30)
...
# MILESTONE AE COMPLETE (local gates: `feature-gate --stack android` + `validate-bootstrap --quick`)
...
## Milestone R – Project Initialization Template Alignment

> **Legend:** `[Agent]` · `[ADB]` · `[Human]` · `[Parallel-*]` · `[Sequential]` — see archived plan for batch definitions.

> **Gate (verified 2026-06-12):** `./gradlew lint test detekt koverXmlReport assembleDebug` green locally (JDK 21); `check-file-limits.ps1 -Fail` green.

### R.1 Agent workspace memory

- [x] **R.1.1** – **R.1.5** — Verdict: ✅ [COMPLETED] — Evidence: `AGENTS.md`, `AGENT_MEMORY.md`, `KNOWLEDGE_BASE.md`, `PROMPT_LIBRARY.md`, `.cursor/rules/`, `docs/adr/`, `docs/DECISION_LOG.md`, README/CONTRIBUTING links.

### R.2 CI/CD & security guardrails

- [x] **R.2.1** – **R.2.6c** — Verdict: ✅ [COMPLETED] — Evidence: CodeQL, grouped Dependabot, `android.yml` pipeline, detekt + baseline, Kover, file-size CI enforcement, UI modularization.
- [x] **R.2.6v** `[Agent+ADB]` — Verdict: ✅ [COMPLETED]
  - Evidence: `./gradlew test` green (Paparazzi verify + unit tests); `./gradlew :app:connectedDebugAndroidTest` green on **OnePlus CPH2583 (Android 16)** 2026-06-12 — 4/4 tests (`MainActivitySmokeInstrumentedTest`, `DeeplinkInstrumentedTest`, `ExampleInstrumentedTest`). Onboarding helper fix in `InstrumentedTestHelpers.kt`.
- [x] **R.2.7** — Verdict: ✅ [COMPLETED]
  - Evidence: `:app` `lint { abortOnError = true }`; `./gradlew :app:lintDebug` passes.
- [x] **R.2.8** – **R.2.10** — Verdict: ✅ [COMPLETED] — Evidence: Paparazzi accessibility snapshots, APK size budget, `pr-title.yml`.
- [x] **R.2.11** `[Agent+Human]` — Verdict: ✅ [COMPLETED] (docs) / `[Human]` enable in GitHub
  - Evidence: [`docs/BRANCH_PROTECTION.md`](BRANCH_PROTECTION.md), CONTRIBUTING + AGENTS.md. **Human:** apply rules in GitHub Settings.

### R.3 F-Droid reproducible builds

- [x] **R.3.1** — Verdict: ✅ [COMPLETED] — Evidence: `SOURCE_DATE_EPOCH` on CI build + release jobs; `docs/REPRODUCIBLE_BUILDS.md`.
- [x] **R.3.2** (Optional) — Verdict: ✅ [COMPLETED]
  - Evidence: Double debug `assembleDebug` SHA-256 compare on tag/release workflow (`android.yml` `release-apk` job). Signed APK byte identity not asserted (documented limitation).

### R.4 Pre-release & dev environment

- [x] **R.4.1** – **R.4.5** — Verdict: ✅ [COMPLETED] — Evidence: `PRE_RELEASE_AUDIT.md`, `GOLDEN_PATH.md`, devcontainer, pre-commit, SECURITY/COC/templates, CODEOWNERS.

### R.5 Pre-release gate — first device pass (2026-06-12)

Ongoing per-tag ritual lives in [`BUILD_PLAN.md`](BUILD_PLAN.md); first **R.5.2** evidence recorded here:

- [x] **R.5.2** Release smoke on device `[ADB]` `[Human]` — Verdict: ✅ [COMPLETED] (first pass)
  - Evidence: **OnePlus CPH2583 (Android 16)** — `MultiAppShare-v1.8.0-release.apk` cold launch OK; `multiappshare://open` → Groups; SEND text → group pick → sharing FGS notification; Export Groups → encrypted passphrase dialog. Full SAF export/import file pick remains human per [`RELEASE_SMOKE.md`](RELEASE_SMOKE.md).

# MILESTONE R COMPLETE ✅

---

## Milestone S – Bug hardening + feature dashboard

> **Legend:** `[Agent]` · `[ADB]` · `[Human]` · `[Parallel-*]` · `[Sequential]`

> **Gate (verified 2026-06-12):** `./gradlew lint test detekt koverXmlReport assembleDebug` green; `check-file-limits.ps1 -Fail` green; `./gradlew :app:assembleRelease` green; `./gradlew :app:connectedDebugAndroidTest` **5/5** on **OnePlus CPH2583 (Android 16)**.

### S.3 Data integrity — Phase S-A / S-B

- [x] **S.3.1** `[P0]` Room group sync on delete/import — Verdict: ✅ [COMPLETED]
  - Evidence: `GroupDao.replaceAllGroups`, `GroupsRepository.saveGroups`; `deleteGroup_persistsAfterReload`, `import_replacesAllGroups` in `GroupsRepositoryTest.kt`.
- [x] **S.3.2** `[P1]` History prune-on-save — Verdict: ✅ [COMPLETED]
  - Evidence: `HistoryDao.replaceAllHistory`; `HistoryRepositoryTest.saveHistory_replacesAndCapsAtFiftyRows`.
- [x] **S.3.3** `[P1]` Unique group names — Verdict: ✅ [COMPLETED]
  - Evidence: `GroupNameHelper`, `GroupMutations.createGroup`; `GroupMutationsTest.createGroup_rejectsDuplicateName`.
- [x] **S.3.4** `[P1]` Corrupt app-list JSON — Verdict: ✅ [COMPLETED]
  - Evidence: `Converters.toAppInfoList` logs via `Log.w`; `ConvertersTest.toAppInfoList_invalidJson_returnsEmptyList`.

### S.5 Backup / export hardening — Phase S-A / S-B

- [x] **S.5.1** `[P0]` Export passphrase race — Verdict: ✅ [COMPLETED]
  - Evidence: `MainActivity` export launcher no longer zeroes passphrase; `BackupOperations` clears in `finally`.
- [x] **S.5.2** `[P1]` Null output stream — Verdict: ✅ [COMPLETED]
  - Evidence: `BackupOperations.exportGroupsToUri` toast on null stream.
- [x] **S.5.3** `[P1]` Import OOM guard — Verdict: ✅ [COMPLETED]
  - Evidence: 4 MB cap before `readText()` in `BackupOperations`.

### S.4 Share session lifecycle — Phase S-C / S-F

- [x] **S.4.1** `[P1]` Rotation persistence — Verdict: ✅ [COMPLETED]
  - Evidence: `ShareSessionState` in `MainViewModel` (Compose `mutableStateOf`).
- [x] **S.4.2** `[P1]` SEND session reset — Verdict: ✅ [COMPLETED]
  - Evidence: `clearSessionShareState()` before applying new SEND in `MainActivity.handleIntent`.
- [x] **S.4.3** `[P2]` FGS teardown — Verdict: ✅ [COMPLETED]
  - Evidence: `SharingService.ACTION_STOP` in `stopSharingService()`.
- [x] **S.4.4** `[P2]` Mutation races — Verdict: ✅ [COMPLETED]
  - Evidence: `Mutex` in `MainViewModelState` for mutations and history writes.

### S.6 Onboarding & deeplinks — Phase S-D / S-F

- [x] **S.6.1** `[P1]` Deeplink expand before load — Verdict: ✅ [COMPLETED]
  - Evidence: `pendingExpandGroupName` queue in `MainViewModelState`; `DeeplinkInstrumentedTest.deeplinkGroup_existingName_expandsGroup`.
- [x] **S.6.2** `[P1]` Onboarding auto-group race — Verdict: ✅ [COMPLETED]
  - Evidence: Removed silent auto-group from `loadData()`; Autofill-only path in UI.
- [x] **S.6.3** `[P2]` Name normalization — Verdict: ✅ [COMPLETED]
  - Evidence: `GroupNameHelper.normalize` shared across mutations and deeplinks.
- [x] **S.6.4** `[P2]` Main-thread Compose state — Verdict: ✅ [COMPLETED]
  - Evidence: `showOnboardingDialog` set via `withContext(Dispatchers.Main)` in `loadData()`.

### S.7 Sharing reliability — Phase S-C

- [x] **S.7.1** `[P1]` Failed share target — Verdict: ✅ [COMPLETED]
  - Evidence: `SharingService.ACTION_SHARE_FAILED` broadcast; auto-advance index.
- [x] **S.7.2** `[P1]` Activity resolution — Verdict: ✅ [COMPLETED]
  - Evidence: `handleIncompatibleApps` prefers stored `activityName` with stable fallback.

### S.1 – S.2 Feature dashboard — Phase S-E

- [x] **S.1** `[P1]` Complete `DashboardViewModel.loadData()` — Verdict: ✅ [COMPLETED]
  - Evidence: `DashboardViewModel.loadData()` → `DashboardUiState.Success`; `DashboardViewModelTest.loadData_emitsSuccessWithGroupsSortedByUsage`.
- [x] **S.2** Migrate dashboard composables — Verdict: ✅ [COMPLETED]
  - Evidence: `DashboardHistoryDialog`, `DashboardAboutDialog` in `:feature-dashboard`; `MainScreenContent` uses feature module dialogs.
  - **S.2.x** `[P3]`: share overlay blur sibling layout; dialog `remember(group.name)` keys; `GroupList` LazyColumn keys.

### S.8 Security & observability — Phase S-F

- [x] **S.8.1** `[P2]` Exclude plaintext JSON from auto-backup — Verdict: ✅ [COMPLETED]
  - Evidence: `data_extraction_rules.xml` excludes `groups.json`, `history.json`.
- [x] **S.8.2** `[P2]` Log JSON shadow backup failures — Verdict: ✅ [COMPLETED]
  - Evidence: `Timber.e` in `GroupsRepository` / `HistoryRepository` catch blocks.

### S.9 Release hygiene — Phase S-F

- [x] **S.9.1** `[P2]` Notification-denied fallback UX — Verdict: ✅ [COMPLETED]
  - Evidence: Incompatible-apps toast when `notificationsEnabled == false` in `MainActivity`.
- [x] **S.9.2** `[P2]` ProGuard + minified smoke — Verdict: ✅ [COMPLETED]
  - Evidence: Removed stale `com.edwardlthompson...` keep from `consumer-rules.pro`; `./gradlew :app:assembleRelease` green 2026-06-12.
