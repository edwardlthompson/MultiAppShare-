...
In Cursor, type `/upgrade`. On a **child** the agent compares this project to the template and writes a plan. It does not apply changes until you name item numbers. Other IDEs: [`docs/help/UPGRADE.md`](help/UPGRADE.md).
...
## Multi App Share (android child) notes

- Board files live under `docs/` (`BUILD_PLAN`, `DECISION_LOG`, `COMPLETED_TASKS`) — keep that layout.
- Primary CI is `.github/workflows/android.yml` (**Android CI**), not `ci.yml`.
- Do **not** adopt release-please or GitHub Pages from the template.
- Keep `.cursor/rules/project.mdc` (product rules) in addition to generated `main.mdc`.
- After cherry-picks: `bash scripts/validate-bootstrap.sh --quick` and `bash scripts/feature-gate.sh --stack android`.
- Alignment / sync records: [`docs/BOOTSTRAP_ALIGNMENT.md`](BOOTSTRAP_ALIGNMENT.md), this file's sync section in the map.
